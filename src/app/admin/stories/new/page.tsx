...
export default function NewStoryPage() {
  ...
  const unclaimedStoriesQuery = query(collection(db, 'stories'), where('status', '==', 'unclaimed'));
  const [stories, isLoading] = useCollectionData(unclaimedStoriesQuery, { idField: 'id' });
                ...
                {isLoading ? (
                    ...
                    <Popover open={open} onOpenChange={setOpen}>
                        <PopoverTrigger asChild>
                            ...
                            >
                            {value
                                ? stories?.find((story) => story.slug === value)?.name.en
                        ...
                        <PopoverContent className="w-[--radix-popover-trigger-width] p-0">
                            <Command>
                                ...
                                {stories?.map((story) => (
                                    ...
                                    />
                                    {story.name[language] || story.name.en}
                                    </CommandItem>
                                ))}
                                </CommandGroup>
                            </Command>
